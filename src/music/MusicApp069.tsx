...
export const VITR_WEB_VERSION = '0.6.9';
      ...
      <main className="frxe-stage frxe069-stage">
        {tab === 'home' && (
          <section className="frxe-screen">
            <header className="frxe-heading frxe069-heading">
              <div><span className="frxe-kicker">VITR WEB · v{VITR_WEB_VERSION}</span><h1>VITR</h1><p>Home · made around your plays, saves, artists, genres and discovery signals.</p></div>
              <a className="frxe-icon-button" href={VITR_REPO} target="_blank" rel="noreferrer" aria-label="Open VITR source"><ExternalLink size={19} /></a>
            </header>
        ...
        {tab === 'search' && (
          <section className="frxe-screen">
            ...
            <div className="frxe-glass strong frxe-search-glass"><form onSubmit={searchSubmit}><SearchIcon size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search songs, artists, genres…" aria-label="Search VITR" /><button type="submit" aria-label="Search music" className="frxe-search-submit" disabled={searching || !query.trim()}>{searching ? <Loader2 size={18} className="frxe-spin" /> : <ArrowDown size={18} />}</button></form></div>
        ...
        {tab === 'library' && (
          <section className="frxe-screen">
            <header className="frxe-heading compact"><div><h2>Library</h2><p>Saved songs, Your Playlists and VITR Mixes stay on this device.</p></div></header>
        ...
        {tab === 'settings' && (
          <section className="frxe-screen"><header className="frxe-heading compact"><div><h2>Settings</h2><p>Playback settings and local VITR data.</p></div></header><div className="frxe-glass strong frxe069-settings"><label><span>Volume <b>{muted ? 0 : volume}%</b></span><input type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(Number(event.target.value))} /></label><button className="frxe069-setting-row" onClick={() => setMuted((value) => !value)}>{muted ? <VolumeX size={18} /> : <Volume2 size={18} />} {muted ? 'Unmute' : 'Mute'}</button><button className="frxe069-setting-row" onClick={() => setShuffle((value) => !value)}><Shuffle size={18} /> Shuffle: {shuffle ? 'On' : 'Off'}</button><button className="frxe069-setting-row" onClick={() => setRepeat((value) => value === 'off' ? 'queue' : value === 'queue' ? 'track' : 'off')}><Repeat2 size={18} /> Repeat: {repeat}</button><button className="frxe069-setting-row" onClick={() => setFinish((value) => value === 'dark' ? 'glass' : 'dark')}><Sparkles size={18} /> Appearance: {finish === 'glass' ? 'Liquid Glass' : 'Modern Dark'}</button><button className="frxe069-setting-row frxe069-reset" onClick={resetWebPlayer}><Trash2 size={18} /> Reset VITR Web</button><p className="frxe069-reset-copy">Clears saved songs, history, playlists, recent searches and player settings from this browser, then starts VITR fresh.</p><div className="frxe069-version"><span>VITR WEB</span><strong>v{VITR_WEB_VERSION}</strong><small>Playback runs directly in the browser with no VITR server token or worker setup.</small></div></div></section>
        )}
      </main>
